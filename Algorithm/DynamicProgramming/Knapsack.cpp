    ...
    for(int M = 1;M <= max_weight;++M){
        for(int i = 1;i <= n;++i){
            ...
            if(weight_packages[i] <= M && DP[M][i] < (DP[M - weight_packages[i]][i - 1] + value[i])){
                //đem so sánh: có cùng tổng M nhưng vật được chọn là vật trước đó[i - 1] với khi ta chọn vật hiện tại có value[i]
                //nếu kết quả tối ưu khi chọn vật hiện tại > kết quả tối ưu của vật trước đó thì ta mới thay đổi, 
                //còn ko thì kết quả tối ưu hiện tại = giá trị tối ưu nhất của sub-problem trước đó
